---
name: llm-wiki
description: Use this skill to work with the LLM-Wiki service as shared durable memory. Trigger it when a task should read, update, or preserve reusable knowledge in tenant wiki documents instead of leaving it only in chat or local scratch files.
---
...
- the task should stay entirely local and should not touch shared workspace knowledge
...
5. Treat tenant or workspace choice as an authenticated context, not a random string to invent on the fly.
6. When writing, always include `author_type`, `author_id`, and a concise `change_summary`.
7. Use namespaces intentionally. Do not dump unrelated content into the same lane.
...
- future commands should reuse the stored server and tenant or workspace context
...
Treat LLM-Wiki as the canonical service. Do not assume CLI-only behavior or local-file-backed semantics when the service already provides tenancy, auth, revision, and audit behavior.
```
